_report = {
  metrics = {
    ...
    compilation_flags = "nbody3DCX:  --intel -x host -Ofast -fsave-optimization-record nbody.c -o nbody3DCX -qmkl=parallel -fveclib=SVML -fheinous-gnu-extensions\n",
    speedup_if_perfect_openmp = "Not Available",
    __name = "run_0",
    nb_loops_80_if_L1 = "Not Available",
    loops_time = 98.74,
    speedup_if_fp_vect = 1,
    speedup_if_perfect_MPI_OMP_PTHREAD_LOAD_DISTRIBUTION = 1.0016025641026,
    speedup_if_perfect_MPI_OMP_PTHREAD = 1.0016025641026,
    ...
    speedup_if_fully_vectorised = 1.2406,
    nb_loops_80_if_fp_vect = 1,
    flow_complexity = 1,
    speedup_if_clean = 1.0033,
    profiled_time = 3.17,
    array_access_efficiency = 20,
    crest_performance = "Not Implemented Yet",
    number_processes = 1,
    speedup_if_FP_only = 2.2973,
    nb_threads = 1,
    application_time = 3.2546060085297,
    compilation_options = 75,
    innerloops_time = 98.74,
  },
  help = {
    ...
    loops_time = "Percentage of the application time spent in loops (excluding loops present in modules not analyzed by the profiler). It represents 3.13 seconds.",
    ...
    profiled_time = "Application profiling duration in seconds<br><b>Too short run</b>: statistical accuracy of profiling results is average (profiled time corresponds to 635 samples)<br><b>Workaround</b>: Rerun with a longer workload and/or use sampling-rate=high/highest in lprof_params",
    ...
    innerloops_time = "Percentage of the application time spent in innermost loops (excluding loops present in modules not analyzed by the profiler). It represents 3.13 seconds.",
  },
  colors = {
  ...
  comp_opt_report = {
    ["nbody3DCX#nbody.c"] = {
      opts = " --intel -x host -Ofast -fsave-optimization-record nbody.c -o nbody3DCX -qmkl=parallel -fveclib=SVML -fheinous-gnu-extensions",
      score = "0.75",
      issues = {"-g is missing, it is needed to have more accurate reports. ", },
    },
  },
}
